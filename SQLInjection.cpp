...
{
  // convert to lowercase to simplify detection
  std::string lower_sql(sql);
  std::transform(lower_sql.begin(), lower_sql.end(), lower_sql.begin(), ::tolower);

  // check for suspicious pattern: 'or' followed by a quoted or numeric value comparison
  if (lower_sql.find(" or ") != std::string::npos &&
      (lower_sql.find"'") != std::string::npos || lower_sql.find("\"") != std::string::npos || lower_sql.find("=") != std::string::npos))
  {
      std::cout << "[SECURITY WARNING] SQL Injection attempt detected. Query blocked.\n";
